...
- (void)drawImageWithContext:(CGContextRef)contextRef {
    ...
    __unused CGGradientRef gradient = CGGradientCreateWithColorComponents(rgb, colors, NULL, sizeof(colors)/(sizeof(colors[0])*4));//形成梯形，渐变的效果
    CGColorSpaceRelease(rgb);
    
    UIImage *drawedImage = UIGraphicsGetImageFromCurrentImageContext();
    self.layer.contents = drawedImage;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [self drawImageWithContext:contextRef];
}
 */
